...
class SegmentationPipeline:
    ...
    def __init__(self) -> None:
        ...
        if isinstance(dataset_classes, list):
            ...
            self.class_names = dataset_classes.get(self.label_type)
        ...
        model_paths = self.config["paths"][model_type]

        base_output_dir = model_paths["base_output_dir"]
        run_name = model_paths.get("run_name", datetime.now().strftime("%Y%m%d_%H%M%S"))
        self.model_save_dir = os.path.join(base_output_dir, model_paths["checkpoint_subdir"])
        self.visualization_dir = os.path.join(base_output_dir, model_paths["visualization_subdir"])
        self.logs_dir = os.path.join(base_output_dir, model_paths["logs_subdir"])
        self.tensorboard_dir = os.path.join(base_output_dir, model_paths["tensorboard_subdir"])
    ...
    def prepare_data(self) -> Tuple[DataLoader, DataLoader, List[np.ndarray], List[np.ndarray], DataPreprocessor]:
        ...
        if self.dataset_name == "flair":
            ...
            flair_cfg = self.config["data"].get("flair", {})
            train_csv = flair_cfg.get("train_csv")
            val_csv = flair_cfg.get("val_csv")
            base_dir = flair_cfg.get("base_dir")
            ...
            if self.image_dir is None or self.mask_dir is None:
                raise RuntimeError("image_dir and mask_dir must be set for non-CSV datasets")
        ...
        if self.dataset_name == "flair":
            train_dataset = self.DatasetClass(train_imgs, train_masks, transform=train_transform)
            val_dataset = self.DatasetClass(val_imgs, val_masks, transform=val_transform)

        else:
            train_img_patches, train_mask_patches = preprocessor.patchify_dataset(train_imgs, train_masks)
            val_img_patches, val_mask_patches = preprocessor.patchify_dataset(val_imgs, val_masks)
